...
from ai_cli.core.config_manager import ConfigManager, ConfigError
...
@pytest.fixture()
def global_dir(tmp_path_factory: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch) -> Path:
...
class TestInit:
    ...
    def test_nested_merge(self, global_dir, project):
        (global_dir / "config.yaml").write_text("model_params:\n  temperature: 0.7\n  max_response_tokens: 512\n")
        (project / _DOT_AI_CLI / "config.yaml").write_text("model_params:\n  max_response_tokens: 1024\n")
        cm = ConfigManager(project, {})
